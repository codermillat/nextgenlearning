...
export default function CourseDetail() {
  ...
  const { getProgramByUniversityAndCourse, getUniversityBySlug, getProgramsByUniversity, loading } = useData();
    ...
    <>
      <SEOHead
        ...
        keywords={[
          ...
          `${program.name} fees`,
          `${program.name} total fees`,
          `${program.name} fees structure`,
          `${university.shortName} ${program.name} fees`,
          `${university.shortName} ${program.name} total fees`,
          program.duration === 4 ? `${university.shortName} ${program.name} total fees 4 years` : '',
          `${program.name} scholarship`,
          `${program.name} admission`,
          `${program.name} review`,
          `${university.name} ${program.name}`,
          `${university.shortName} ${program.name}`,
          `${university.shortName} ${program.field} fees`,
          'study in India',
          'Bangladeshi students',
          program.degree,
          program.field,
          `${university.shortName} nirf ranking`,
          `${university.shortName} naac grade`
        ].filter(Boolean)}
        url={courseUrl}
        canonical={courseUrl}
      />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6 md:py-8">
        ...
        <section className="mb-6 sm:mb-8">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 sm:mb-6 gap-2">
            <h2 className="text-xl sm:text-2xl font-bold">Other Courses at {university.name}</h2>
            <Link
              to={`/universities/${university.slug}/courses`}
